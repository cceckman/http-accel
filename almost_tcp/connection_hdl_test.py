...
from connection_hdl import Connection
...
class Everything(Component):

    inbuf: In(stream.Signature(8))

    def elaborate(self, platform):
        m = Module()
        m.submodules.read = read_stop = ReadPacketStop(id=3)
        m.submodules.conn = conn = Connection(id=3, bufsz=512)

        connect(m, read_stop.packet, conn.packet_in)
