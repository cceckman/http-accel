...
class Platform(FomuPVTPlatform, LUNAPlatform):
    """
    Mixin of the Fomu platform with the LUNA mixin."
    """
    pass
...
class USBDeviceExample(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        decimator = Signal(2, reset=1)
        m.d.comb += [usb.clk.eq(~decimator.any())]
        m.d.sync += [decimator.eq(decimator + 1)]

        # Create our USB device interface...
        # This doesn't "just" work because we don't have a
        # USB macrocell -- just PHYs.
        # Does the mixin
...
if __name__ == "__main__":
    Platform().build(USBDeviceExample(), do_program=True)
